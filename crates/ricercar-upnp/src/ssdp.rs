...
fn device_entries(dev: &Device) -> Vec<(String, String)> {
    let host = format!("upnp://{}", dev.udn);
    let mut v = vec![
        ("upnp:rootdevice".into(), format!("upnp:rootdevice::{host}")),
        (format!("uuid:{}", dev.udn), format!("uuid:{}", dev.udn)),
    ];
    match dev.kind {
        Kind::Renderer => {
            v.push((
            ...
            for svc in ["AVTransport", "RenderingControl", "ConnectionManager"] {
                v.push((
                    format!("urn:schemas-upnp-org:service:{svc}:1"),
                    format!("urn:schemas-upnp-org:service:{svc}:1::{host}"),
                ));
            }
        }
        Kind::Server => {
            v.push((
                ...
                format!("urn:schemas-upnp-org:device:MediaServer:1::{host}"),
            ));
...
impl Ssdp {
    ...
    pub fn start(devices: Vec<Device>) -> Option<Ssdp> {
        let sock = bind_ssdp_socket()?;
        let stop = Arc::new(AtomicBool::new(false));
        let bootid: u64 = std::process::id() as u64 * 1000;
        let entries: Vec<(String, String, String)> = devices
            .iter()
            .flat_map(|d| {
                device_entries(d)
                    .into_iter()
                    .map(move |(nt, usn)| (nt, usn, d.location.clone()))
            })
            .collect();
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let mut i = 0usize;
                loop {
                    for (nt, usn, loc) in &entries_a {
                        let pkt = alive_packet(nt, usn, loc, bootid);
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                loop {
                    ...
                    for (nt, usn, loc) in &entries_r {
                        if st == "ssdp:all" || &st == nt {
                            let pkt = search_reply(nt, usn, loc, bootid);
